...
func writeCookiesToNetscapeFile(headerString string) (string, error) {
	// Create temp file
	...
	for _, pair := range pairs {
		...
		// For YouTube cookies, assume .youtube.com domain
...
func (m *CookieManager) GetCookieArgs(ctx context.Context, tool string, url string) []string {
	...
	case CookieModeManual:
		...
		if header != "" {
			...
			// For yt-dlp, convert header string to Netscape cookie file format
			// This is more reliable than --add-headers for complex cookie strings
			if tool == "yt-dlp" {
				netscapeFile, err := writeCookiesToNetscapeFile(header)
